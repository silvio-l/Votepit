 ...
 * user + account role" with "bearer token that authorizes exactly ONE board".
 *
...
final readonly class ApiTokenAuthenticator
{
     ...
     * the resolved scope (account_id, board_id, token_id, created_by_user_id,
     * label) — or null on a missing/malformed header, or an unknown or
     * revoked token.
     *
     * Fail-secure: every doubtful case (no header, wrong scheme, lookup
     * error) ends in null, NEVER in an implicit "let through".
     *
     * @return array{token_id: int, account_id: int, board_id: int, created_by_user_id: int, label: string}|null
     */
