...
final readonly class RateLimiter
{
    ...
    public function hit(string $bucket, int $limit, int $windowSeconds): bool
    {
        ...
        return $this->incrementAndFetch($bucket, $windowSeconds) <= $limit;
    }

    /**
     * Increments $bucket in its fixed window and returns the resulting
     * count — same fixed-window semantics as hit(), but for callers that
     * need the raw number rather than an allow/deny decision (e.g.
     * MailVolumeMonitor alerting on absolute outbound-mail volume, not
     * blocking any send).
     *
     * @throws Exception
     */
    ...
    {
        return $this->incrementAndFetch($bucket, $windowSeconds);
    }

    /** @throws Exception */
    private function incrementAndFetch(string $bucket, int $windowSeconds): int
    {
