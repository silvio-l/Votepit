 ...
 * migrations/0028_add_user_scoped_notifications.sql and
 * migrations/0041_add_operator_scoped_notifications.sql for the shape
 * rationale). Four kinds share one table via `scope`:
 ...
 *   - 'user': targets exactly one user (e.g. a comment on their idea).
 *   - 'operator': targets every user with is_operator OR is_support (e.g. a
 *     customer's support ticket/reply) — checked against `users` at query
 *     time, not stored per-recipient, same "role, not membership" shape
 *     'account' already uses via account_members.
 *
...
final readonly class NotificationRepository
{
    ...
    /**
     * Single-recipient notification (notification-preferences PRD:
     * 'idea_comment'/'thread_reply'). $accountId is carried alongside
     * $userId even though targeting is purely by user — see
     * migrations/0028_add_user_scoped_notifications.sql for why.
     *
     * @throws DbalException
     */
    public function createForUser(int $accountId, int $userId, string $type, string $title, string $body, ?string $linkPath): int
    {
        $this->conn->insert('notifications', [
            'scope'      => 'user',
            'account_id' => $accountId,
    ...
    /**
     * Targets every user with is_operator OR is_support (platform staff),
     * regardless of account membership — e.g. a new/replied-to customer
     * support ticket. Unlike createForAccount(), there is no single account
     * to scope to (a ticket queue spans every tenant).
     *
     * @throws DbalException
     */
    public function createForOperators(string $type, string $title, string $body, ?string $linkPath): int
    {
        $this->conn->insert('notifications', [
            'scope'      => 'operator',
            'account_id' => null,
     ...
     * of, plus operator-scoped notifications if the user is an operator or
     * support agent — minus any this user has dismissed from their own
     * inbox. Newest first, with the user's own read state joined in.
     *
            ...
            'SELECT n.id, n.scope, n.account_id, n.type, n.title, n.body, n.link_path, n.created_at,
             ...
             WHERE d.user_id IS NULL
               AND (n.scope = \'broadcast\'
                    OR n.scope = \'user\' AND n.user_id = :user_id
                    OR n.scope = \'account\' AND n.account_id IN (SELECT account_id FROM account_members WHERE user_id = :user_id)
                    OR n.scope = \'operator\' AND EXISTS (
                        SELECT 1 FROM users u WHERE u.id = :user_id AND (u.is_operator = 1 OR u.is_support = 1)
                          AND (
                            (n.type = \'abuse_report_submitted\' AND u.notify_abuse_report_inapp = 1)
                            OR (n.type = \'support_reply\' AND u.notify_support_ticket_inapp = 1)
                          )
                    ))
