 ...
 *
 * A submitted report also fans out an 'abuse_report_submitted' notification
 * to every operator/support agent (scope 'operator', same shape as the
 * support-ticket fan-out in SupportRequestAction — migrations/
 * 0041_add_operator_scoped_notifications.sql), gated per-recipient by their
 * own notify_abuse_report_inapp/email preference (migrations/
 * 0045_add_operator_notification_preferences.sql): the in-app row is always
 * written (NotificationRepository::listForUser() filters it per-viewer at
 * read time), while email is fanned out here, once, to every operator who
 * opted in and has a confirmed notification_email.
 */
final readonly class AbuseReportAction
{
    ...
    public function submit(
        ...
        // This endpoint is anonymous (DSA Art. 16 reporting must not require
        // an account) and already persisted the report above — an SMTP
        // failure here must not turn that success into a 500, which would
        // just prompt the reporter to resubmit the same report again
        // (security-review finding, 2026-09-05). The in-app notification
        // just created is the reliable channel; email is best-effort.
        try {
            foreach ($this->users->findOperatorEmailRecipients('notify_abuse_report_email') as $recipientEmail) {
                $this->notificationMailer->send($recipientEmail, 'New content report', $reason, '/operator', $boardId);
            }
        } catch (\Throwable $e) {
            $this->audit->log('abuse_report.notification_mail_failed', [
                'report_id' => $reportId,
                'error'     => $e->getMessage(),
            ]);
        }
