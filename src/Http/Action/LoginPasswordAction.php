 ...
 *
 * Either success path clears the IP's rate-limit bucket (same
 * "login:password:ip:<addr>" bucket as the RateLimitMiddleware on this
 * route, see AppFactory) — a correct password entered after a handful of
 * typos must not still burn through the remaining budget and get a normal
 * user 429'd. Login2faAction does the analogous reset for its own
 * "login:2fa:ip:<addr>" bucket — the two no longer share a bucket (review
 * 2026-09-04, item 1).
 */
final readonly class LoginPasswordAction
{
    ...
    private function resetIpRateLimit(ServerRequestInterface $request): void
    {
        $ip = ClientIp::resolve($request, $this->trustCloudflareIp);
        if ($ip !== null) {
            $this->rateLimiter->reset('login:password:ip:' . $ip);
        }
    }
