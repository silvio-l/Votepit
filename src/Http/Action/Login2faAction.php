...
final readonly class Login2faAction
{
    ...
    public function __invoke(
        ...
        // RateLimitMiddleware on this route, see AppFactory). Own bucket
        // namespace ("login:2fa:ip:<addr>"), independent of
        // LoginPasswordAction's "login:password:ip:<addr>" (review
        // 2026-09-04, item 1).
        $ip = ClientIp::resolve($request, $this->trustCloudflareIp);
        if ($ip !== null) {
            $this->rateLimiter->reset('login:2fa:ip:' . $ip);
        }
