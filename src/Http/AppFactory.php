...
final class AppFactory
{
    ...
    public static function create(
        ...
        // renders no operator-specific legal links.
        ...
        if ($conn instanceof Connection) {
            ...
            $smtpResolver     = new SmtpConfigResolver($smtpSettingsRepo, $boardSmtpRepo, $encryptionSvc, $config->smtp, $smtpHostPolicy);
            ...
            ) use ($config, $accountMemberRepo, $userRepo, $bootstrapFeatures): ResponseInterface {
                $csrfToken = $request->getAttribute(CsrfMiddleware::ATTR_TOKEN);
                $user      = $request->getAttribute(AuthNMiddleware::ATTR_USER);
                $userPayload = null;
                if (is_array($user)) {
            ...
            $app->post('/login', function (
                ...
                if ($email !== '' && filter_var($email, FILTER_VALIDATE_EMAIL) !== false) {
                    ...
                    // Resolve per-board SMTP: use the first path segment of returnTo as the slug.
                    // Account context comes from the request attribute (AccountContextMiddleware
                    // already runs before this in the global pipeline).
                    $boardId = null;
                    if ($returnTo !== '') {
                        $firstSeg = explode('/', ltrim($returnTo, '/'))[0];
                        if ($firstSeg !== '') {
                            $accountId = (int) $request->getAttribute(AccountContextMiddleware::ATTR_ACCOUNT_ID);
                            $boardRow  = $boardRepo->findBySlugForAccount($firstSeg, $accountId);
            ...
            $app->post('/login/password', new LoginPasswordAction($userRepo, $hasher, $tokenRepo, $vault, $loginSessionIssuer, $audit))
                ->add(AuthZMiddleware::anon($responseFactory))
            ...
            $app->post(
                ...
                new PasswordResetRequestAction($userRepo, $hasher, $tokenRepo, $vault, $mailer, $smtpResolver, $config, $audit),
            )
            ...
            $app->get('/api/v1/board', $apiBoardAction)
                ...
                ->add(new ApiTokenAuthMiddleware($apiTokenAuthenticator, $responseFactory));
            ...
            $publicProfileAction = new PublicProfileAction($userRepo, $userSocialLinkRepo, $accountMemberRepo);
            ...
            $app->get($accountPrefix . '/admin/boards', (new BoardListAction($boardRepo, $accountRepoForPlan))->list(...))
            ...
            $app->get($accountPrefix . '/{board}/ideas/{id:[0-9]+}', new IdeaDetailAction($boardRepo, $ideaRepo, $commentRepo, $accountMemberRepo))
                ->add(AuthZMiddleware::anon($responseFactory));
            ...
            // AuthZ: accountAdmin (board-scoped admin mutation, anon → 401,
            ...
            $app->post($accountPrefix . '/{board}/ideas/{id:[0-9]+}/status', new IdeaStatusAction($boardRepo, $ideaRepo, new StatusService(), $audit))
            ->add(AuthZMiddleware::accountAdmin($responseFactory, $accountMemberRepo))
            ->add(RateLimitMiddleware::perAction(
                new RateLimiter($conn),
                $responseFactory,
            ...
            $app->post(
                ...
                new CommentCreateAction($boardRepo, $ideaRepo, $commentRepo, $audit, $moderation, $blockRepoForCheck, $modConfigRepo),
            )
            ...
            // (admin-only moderation). AuthZ: accountAdmin; CSRF globally
            ...
            // AuthZ: accountAdmin (owner + moderator may read).
            ...
            $memberAction = new MemberAction($accountMemberRepo, $inviteRepo, $audit);
            ...
            // accountAdmin — every account_members role is owner|moderator,
            // so this is effectively "any team member of the account",
            // matching "a customer contacts us from their dashboard" rather
            // than being owner-only like export/delete above.
            ...
            // GET  /admin/boards/{slug}/tokens             — list tokens.
            // POST /admin/boards/{slug}/tokens             — create a token (plaintext only in this response).
            // POST /admin/boards/{slug}/tokens/{id}/revoke — revoke a token.
            // AuthZ: accountAdmin (owner + moderator — mirrors branding/moderation/board-SMTP, see ApiTokenAction doc).
            // $apiTokenAction was already constructed further up (cloud-routing
            // safety net, see there) — only referenced here.
            $app->get($accountPrefix . '/admin/boards/{slug}/tokens', $apiTokenAction->list(...))
                ->add(AuthZMiddleware::accountAdmin($responseFactory, $accountMemberRepo));

            $app->post($accountPrefix . '/admin/boards/{slug}/tokens', $apiTokenAction->create(...))
                ->add(AuthZMiddleware::accountAdmin($responseFactory, $accountMemberRepo));

            $app->post($accountPrefix . '/admin/boards/{slug}/tokens/{id:[0-9]+}/revoke', $apiTokenAction->revoke(...))
                ->add(AuthZMiddleware::accountAdmin($responseFactory, $accountMemberRepo));
