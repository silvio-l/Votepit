...
use Votepit\Http\Action\IdeaTagAssignAction;
use Votepit\Http\Action\IdeaTagRemoveAction;
...
use Votepit\Http\Action\OAuthCallbackAction;
use Votepit\Http\Action\OAuthStartAction;
...
use Votepit\Http\Action\TagCreateAction;
use Votepit\Http\Action\TagDeleteAction;
use Votepit\Http\Action\TagListAction;
use Votepit\Http\Action\TagUpdateAction;
...
use Votepit\Security\OAuth\CurlOAuthHttpClient;
...
use Votepit\Security\Webhook\CurlWebhookSender;
...
final class AppFactory
{
    ...
    public static function create(
        ...
        // OAuth login (Google/GitHub) outbound HTTP seam — null = the real
        // CurlOAuthHttpClient (production/self-host default). Tests inject
        // an InMemoryOAuthHttpClient to exercise the token-exchange/
        // profile-fetch contract without real network calls, same pattern
        // as $mailer/$smtpHostPolicy above.
        ?OAuthHttpClient $oauthHttpClient = null,
        // board-webhooks: injectable HTTP sender + defer-runner so tests can
        // exercise the full HTTP route without real network access and
        // without waiting on PHP process shutdown (see WebhookDispatcher).
        ?WebhookSenderInterface $webhookSender = null,
        ?\Closure $webhookDeferRunner = null,
    ): App {
        ...
        if ($conn instanceof Connection) {
            ...
            // OAuth login (Google/GitHub) outbound HTTP seam — see create()'s
            // $oauthHttpClient doc comment.
            $oauthHttp          = $oauthHttpClient ?? new CurlOAuthHttpClient();
            $totp               = new Totp();
            ...
            $tagRepo       = new TagRepository($conn);
            $timeTrap      = new TimeTrapService($config->appKey);
            $normalizer    = new TitleNormalizer();

            // board-webhooks: own EncryptionService context ('board_webhook',
            // not 'smtp'/'totp') — key separation between at-rest secrets,
            // same discipline as the TOTP encryption above. WebhookUrlGuard
            // is ALWAYS restrictive (unlike SmtpHostPolicy) — see its class
            // doc for why self-host isn't exempt here.
            $webhookRepo       = new BoardWebhookRepository($conn);
            $webhookSecretEnc  = new EncryptionService($config->appKey, 'board_webhook');
            $webhookUrlGuard   = new WebhookUrlGuard();
            $webhookDispatcher = new WebhookDispatcher(
                $webhookRepo,
                $webhookUrlGuard,
                $webhookSender ?? new CurlWebhookSender(),
                $webhookSecretEnc,
                $webhookDeferRunner,
                null,
                new RateLimiter($conn),
    ...
    private static function registerAuthRoutes(
        ...
        $app->get('/api/bootstrap', function (
            ...
            $response->getBody()->write((string) json_encode([
                ...
                // OAuth login (Google/GitHub) — only providers with BOTH
                // client_id/client_secret configured (Config::
                // oauthProviderConfigured()) appear here, so the SPA simply
                // renders no button for an unconfigured provider instead of
                // a broken one (LoginPage.tsx). Not sensitive: same value
                // for every visitor, no secret is ever exposed.
                'oauth_providers'       => $config->configuredOAuthProviders(),
            ]));
            return $response->withStatus(200)->withHeader('Content-Type', 'application/json');
        })->add(AuthZMiddleware::anon($responseFactory));
        ...
        // GET /login/oauth/{provider}/start + GET /login/oauth/{provider}/callback
        // — OAuth2 login (Google/GitHub), additive to magic-link/password/TOTP,
        // landing on the SAME LoginSessionIssuer. AuthZ: anon. Both fail-secure
        // 404 for an unknown or unconfigured provider (OAuthStartAction/
        // OAuthCallbackAction check Config::oauthProviderConfigured()
        // themselves) — a self-host installation with no oauth_providers
        // config simply never exposes these routes' functionality, same
        // posture as every other opt-in config-gated feature here.
        $oauthStateRepo    = new OAuthStateRepository($conn);
        $oauthIdentityRepo = new OAuthIdentityRepository($conn);

        $app->get('/login/oauth/{provider}/start', new OAuthStartAction($config, $oauthStateRepo, $vault, $audit))
            ->add(AuthZMiddleware::anon($responseFactory));

        // Rate limit mirrors login:password/login:2fa (per-IP, config-driven,
        // fails closed to Config::UNCONFIGURED_ACTION_LIMIT if unset) — a
        // provider-error/replay loop against this endpoint is otherwise
        // unthrottled request volume just like any other login route.
        $oauthCallbackRateLimit  = $config->rateLimit('login:oauth');
        $oauthCallbackRateLimiter = new RateLimiter($conn);
        $app->get(
            '/login/oauth/{provider}/callback',
            new OAuthCallbackAction($config, $oauthStateRepo, $oauthIdentityRepo, $userRepo, $tokenRepo, $vault, $hasher, $oauthHttp, $loginSessionIssuer, $audit, $conn),
        )
            ->add(AuthZMiddleware::anon($responseFactory))
            ->add(RateLimitMiddleware::perIp(
                ...
                $responseFactory,
                'login:oauth',
                $oauthCallbackRateLimit['limit'],
                $oauthCallbackRateLimit['window'],
                $config->trustCloudflareIp,
            ));
    ...
    private static function registerBoardRoutes(
        ...
        // Comment-reactions (migrations/0050_add_comment_reactions.sql):
        // one instance shared by the detail-page read (IdeaDetailAction)
        // and the toggle write (CommentReactionAction) below.
        $reactionRepo = new CommentReactionRepository($conn);
        ...
        $app->get($accountPrefix . '/{board}', new BoardHomeAction($boardRepo, $ideaRepo, $accountMemberRepo, $accountRepoForPlan, $planPolicy, $tagRepo))
            ->add(AuthZMiddleware::anon($responseFactory));
        ...
        $app->get($accountPrefix . '/{board}/ideas/{id:[0-9]+}', new IdeaDetailAction($boardRepo, $ideaRepo, $commentRepo, $reactionRepo, $accountMemberRepo, $viewTracker, $tagRepo))
            ->add(AuthZMiddleware::anon($responseFactory));
        ...
        $app->post($accountPrefix . '/{board}/ideas', new IdeaCreateAction($boardRepo, $ideaRepo, $normalizer, $audit, $moderation, $timeTrap, $blockRepoForCheck, $modConfigRepo, $webhookDispatcher))
        ...
        $app->post(
            ...
            new IdeaStatusAction($conn, $boardRepo, $ideaRepo, new StatusService(), $audit, $notificationRepo, $userRepo, $voteRepo, $commentRepo, $statusNotificationMailer, $blockRepoForCheck, $webhookDispatcher),
        )
        ...
        // AuthZ: anon. Unknown/private board → 404 (findPublicBySlugForAccount,
        // same visibility gate as BoardHomeAction/IdeaDetailAction).
        $app->get($accountPrefix . '/{board}/tags', new TagListAction($boardRepo, $tagRepo, $accountMemberRepo))
            ->add(AuthZMiddleware::anon($responseFactory));

        // POST /{board}/tags — create a tag.
        // AuthZ: accountModerate (owner|admin|moderator, anon → 401,
        // missing account role → 403); CSRF global. Duplicate name in the
        // same board → 422 (TagRepository::create() UNIQUE(board_id, name)).
        $app->post($accountPrefix . '/{board}/tags', new TagCreateAction($boardRepo, $tagRepo, $audit))
            ->add(AuthZMiddleware::accountModerate($responseFactory, $accountMemberRepo));

        // POST /{board}/tags/{id} — rename/recolor a tag.
        // AuthZ: accountModerate; CSRF global. Board-scoping in the action
        // via TagRepository::findInBoard (foreign tag → 404, no mutation).
        $app->post($accountPrefix . '/{board}/tags/{id:[0-9]+}', new TagUpdateAction($boardRepo, $tagRepo, $audit))
            ->add(AuthZMiddleware::accountModerate($responseFactory, $accountMemberRepo));

        // POST /{board}/tags/{id}/delete — delete a tag (cascades assignments).
        // AuthZ: accountModerate; CSRF global. Board-scoping via a board-scoped
        // WHERE clause in TagRepository::delete() (foreign tag → 404, no mutation).
        $app->post($accountPrefix . '/{board}/tags/{id:[0-9]+}/delete', new TagDeleteAction($boardRepo, $tagRepo, $audit))
            ->add(AuthZMiddleware::accountModerate($responseFactory, $accountMemberRepo));

        // POST /{board}/ideas/{id}/tags — assign a tag to an idea.
        // AuthZ: accountModerate; CSRF global. Both the idea and the tag are
        // loaded board-scoped (foreign idea/tag → 404, no mutation, and no
        // cross-board tag assignment even within the same account).
        $app->post($accountPrefix . '/{board}/ideas/{id:[0-9]+}/tags', new IdeaTagAssignAction($boardRepo, $ideaRepo, $tagRepo, $audit))
            ->add(AuthZMiddleware::accountModerate($responseFactory, $accountMemberRepo));

        // POST /{board}/ideas/{id}/tags/{tagId}/remove — remove a tag from an idea.
        // AuthZ: accountModerate; CSRF global. Same board-scoping as the assign route above.
        $app->post($accountPrefix . '/{board}/ideas/{id:[0-9]+}/tags/{tagId:[0-9]+}/remove', new IdeaTagRemoveAction($boardRepo, $ideaRepo, $tagRepo, $audit))
            ->add(AuthZMiddleware::accountModerate($responseFactory, $accountMemberRepo));
        ...
        // POST /{board}/ideas/{id}/comments/{commentId}/react — toggle an
        // emoji reaction on a comment. AuthZ: user (anon → 401); CSRF +
        // BlockCheck global; per-action rate limit comment:react (mirrors
        // idea:vote's generosity — reactions are just as lightweight as
        // votes). Board-/idea-/comment-scoping in the action (foreign
        // idea/comment → 404, no mutation). One active reaction per
        // (comment, user) — mirrors idea:vote's toggle/switch/retract.
        $reactRateLimit = $config->rateLimit('comment:react');

        $app->post(
            $accountPrefix . '/{board}/ideas/{id:[0-9]+}/comments/{commentId:[0-9]+}/react',
            new CommentReactionAction($boardRepo, $ideaRepo, $commentRepo, $reactionRepo, $audit, $blockRepoForCheck),
        )
            ->add(AuthZMiddleware::user($responseFactory))
            ->add(RateLimitMiddleware::perAction(
                new RateLimiter($conn),
                $responseFactory,
                'comment:react',
                $reactRateLimit['limit'],
                $reactRateLimit['window'],
                static function (ServerRequestInterface $r): ?string {
                    $user = $r->getAttribute(AuthNMiddleware::ATTR_USER);
                    return is_array($user) ? (string) ($user['id'] ?? '') : null;
                },
    ...
    private static function registerBoardAdminRoutes(
        ...
        BoardWebhookRepository $webhookRepo,
        WebhookUrlGuard $webhookUrlGuard,
        EncryptionService $webhookSecretEnc,
    ): void {
        ...
        // board-webhooks: available in both self-host and cloud mode (unlike
        // board SMTP above) — every board owner/admin may configure an
        // outbound webhook regardless of routing mode.
        $boardWebhookAction = new BoardWebhookAction($boardRepo, $webhookRepo, $webhookUrlGuard, $webhookSecretEnc, $audit);

        $app->get($accountPrefix . '/admin/boards/{slug}/webhook', $boardWebhookAction->get(...))
            ->add(AuthZMiddleware::accountAdmin($responseFactory, $accountMemberRepo));

        $app->put($accountPrefix . '/admin/boards/{slug}/webhook', $boardWebhookAction->put(...))
            ->add(AuthZMiddleware::accountAdmin($responseFactory, $accountMemberRepo));

        $app->patch($accountPrefix . '/admin/boards/{slug}/webhook', $boardWebhookAction->patch(...))
            ->add(AuthZMiddleware::accountAdmin($responseFactory, $accountMemberRepo));

        $app->delete($accountPrefix . '/admin/boards/{slug}/webhook', $boardWebhookAction->delete(...))
            ->add(AuthZMiddleware::accountAdmin($responseFactory, $accountMemberRepo));
