-- 0000_baseline — Migrations-Baseline (Sprint 1.1)
--
-- 1:1-Kopie von db/schema.sql zum Zeitpunkt der Einführung des
-- Migrations-Runners. Idempotent (CREATE TABLE IF NOT EXISTS), damit sie auf
-- Frischinstallationen wie auf bereits per `mysql < db/schema.sql`
-- aufgesetzten Installationen gleichermaßen sicher läuft.
--
-- WICHTIG: Ab jetzt gilt — diese Datei UND db/schema.sql werden NIEMALS mehr
-- nachträglich editiert. Jede künftige Schema-Änderung ist eine NEUE Datei
-- unter core/migrations/ (siehe migrations/README.md).
...
-- app_settings: installations-weite Key-Value-Einstellungen (z. B. SMTP).
-- Passwort-Felder: verschlüsselt at rest (sodium_crypto_secretbox via EncryptionService).
...
-- (ADR-3-Amendment — siehe VoteRepository).
