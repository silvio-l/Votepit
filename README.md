...
[![GitHub stars](https://img.shields.io/github/stars/silvio-l/Votepit?style=social)](https://github.com/silvio-l/Votepit/stargazers)
...
- Passwordless **magic-link auth** (email), persistent session, rate limiting — with an optional password and TOTP-based 2FA any user can self-enable in their profile, or sign in with **Google/GitHub OAuth**
- Submit, edit, and withdraw ideas, with board-scoped **tags**; comments with **emoji reactions**
...
- **Agent API & MCP**: board-scoped, rate-limited REST API and an MCP endpoint for AI agents, secured by admin-issued API tokens — plus [`@votepit/cli`](packages/cli/README.md), a zero-dependency CLI for the same API
- **Outbound webhooks** per board (idea created / status changed), HMAC-signed and SSRF-guarded, for wiring your own Slack/Discord/CI notifications

Not included: an embeddable widget, email notifications (the notification inbox above is in-app only), or realtime updates.
