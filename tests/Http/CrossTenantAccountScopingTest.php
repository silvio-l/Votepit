...
final class CrossTenantAccountScopingTest extends IntegrationTestCase
{
    ...
    // -------------------------------------------------------------------------
    // 3. Moderator of the default account → 403 for board-management routes
    //    outright, since moderator no longer passes accountAdmin at all (see
    //    AuthZMiddleware::accountAdmin() — narrowed to owner|admin). Never
    //    even reaches the cross-tenant board lookup.
    // -------------------------------------------------------------------------

    public function test_moderator_role_gets_403_for_board_management_regardless_of_account(): void
    {
    ...
    // -------------------------------------------------------------------------
    // 3b. Admin (not owner) of the default account → 404 on a board that
    //    belongs to a foreign account (no owner-only exception; admin now
    //    holds the role that moderator held here before).
    // -------------------------------------------------------------------------

    public function test_admin_role_also_gets_404_for_board_in_foreign_account(): void
    {
        $foreignAccountId = $this->insertAccount(['slug' => 'acct-foreign-admin', 'name' => 'Foreign Account']);
        $this->insertBoard('admin-secret', ['account_id' => $foreignAccountId]);

        $adminId = $this->seedUser('admin-scoping@example.com');
        $this->insertAccountMember($this->defaultAccountId(), $adminId, 'admin');

        $response = $this->createApp()->handle(
            $this->getRequest('/admin/boards/admin-secret/branding', $adminId),
        );
