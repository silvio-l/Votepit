...
final class LoginPasswordAndTwoFaActionTest extends IntegrationTestCase
{
    ...
    /**
     * A correct login after a handful of prior typos/retries must not still
     * carry those attempts' weight against the same IP's budget — otherwise
     * completely normal use (typo, then the right password) can 429 a real
     * user well before any actual brute-force attempt gets close to the
     * limit. Mirrors Login2faAction's identical reset-on-success behavior.
     */
    public function test_successful_password_login_resets_the_rate_limit_bucket_for_the_ip(): void
    {
        $this->userWithPassword('resets-limit@example.com', 'correct-password-1');
        $app = $this->createApp();
        $ip  = '203.0.113.10';

        // Config default: 10/900s (see testConfig() override above).
        for ($i = 0; $i < 9; $i++) {
            $response = $app->handle($this->post('/login/password', [
                'email'    => 'resets-limit@example.com',
                'password' => 'wrong-password',
            ], $ip));
            self::assertSame(401, $response->getStatusCode());
        }

        $success = $app->handle($this->post('/login/password', [
            'email'    => 'resets-limit@example.com',
            'password' => 'correct-password-1',
        ], $ip));
        self::assertSame(200, $success->getStatusCode());

        // Without the reset, this batch would push the bucket's count from
        // 10 (9 failures + the success) to 19, well past the limit of 10.
        $statusCodes = [];
        for ($i = 0; $i < 9; $i++) {
            $request       = $this->post('/login/password', [
                'email'    => 'resets-limit@example.com',
        ...
        self::assertNotContains(429, $statusCodes);
    }
