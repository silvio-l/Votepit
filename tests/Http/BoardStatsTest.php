 ...
 *
 * Covered:
 *  - stats block present (weekly_votes / weekly_new_ideas / avg_consensus / total_ideas)
 *  - weekly_votes only counts votes from the last 7 days
 *  - avg_consensus = average of the per-idea approval rate
 *  - total_ideas is the all-time (not just "this week") board idea count
...
final class BoardStatsTest extends IntegrationTestCase
{
    ...
    public function test_stats_block_is_present_with_weekly_values(): void
    {
        ...
        self::assertSame(1, $data['stats']['total_ideas']);
    }

    public function test_total_ideas_counts_all_time_not_just_this_week(): void
    {
        $boardId = $this->insertBoard('stats-total');
        $u1      = $this->insertUser('t1@example.com');

        $old = $this->seedIdea($boardId, $u1, 'An old idea');
        $this->conn->update('ideas', ['created_at' => (new \DateTimeImmutable('-30 days'))->format('Y-m-d H:i:s')], ['id' => $old]);
        $this->seedIdea($boardId, $u1, 'A fresh idea');

        $data = $this->decode($this->createApp()->handle($this->getBoardRequest('stats-total')));

        self::assertSame(1, $data['stats']['weekly_new_ideas'], 'Only the fresh idea is within the 7-day window.');
        self::assertSame(2, $data['stats']['total_ideas'], 'total_ideas must count both regardless of age.');
    }
    ...
    public function test_stats_are_board_scoped(): void
    {
        ...
        self::assertSame(0, $data['stats']['total_ideas'], 'Board B has no ideas — must not see board A\'s idea.');
    }
    ...
    public function test_empty_board_returns_zero_stats(): void
    {
        ...
        self::assertSame(0, $data['stats']['weekly_new_ideas']);
        self::assertSame(0, $data['stats']['avg_consensus']);
        self::assertSame(0, $data['stats']['total_ideas']);
    }
}
