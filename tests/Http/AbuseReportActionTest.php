...
final class AbuseReportActionTest extends IntegrationTestCase
{
    ...
    {
        return $this->insertUser('operator-report@example.com', [
            'is_operator'     => 1,
    ...
    public function test_submitting_a_report_notifies_operators_in_app(): void
    {
        $operatorId = $this->operator();
        $app        = $this->createApp();
        ...
        $notification = $this->conn->fetchAssociative(
            "SELECT * FROM notifications WHERE type = 'abuse_report_submitted' AND scope = 'operator'",
        );
        self::assertIsArray($notification);
        self::assertNull($notification['account_id']);
        self::assertSame('/operator', $notification['link_path']);

        $operatorInbox = $app->handle($this->operatorGet('/notifications', $operatorId));
        $operatorData  = json_decode((string) $operatorInbox->getBody(), true);
        self::assertCount(1, $operatorData['notifications']);
        self::assertSame('abuse_report_submitted', $operatorData['notifications'][0]['type']);
    }

    public function test_operator_who_opted_out_of_abuse_report_inapp_does_not_see_it(): void
    {
        $operatorId = $this->insertUser('operator-opted-out@example.com', [
            'is_operator'                => 1,
            'totp_enabled_at'            => (new \DateTimeImmutable())->format('Y-m-d H:i:s'),
            'notify_abuse_report_inapp'  => 0,
        ]);
        $app = $this->createApp();

        $app->handle($this->anonPost('/reports', [
            'url'    => '/demo/ideas/1',
            'reason' => 'Contains offensive content and personal attacks.',
        ]));
        ...
        $operatorInbox = $app->handle($this->operatorGet('/notifications', $operatorId));
        $operatorData  = json_decode((string) $operatorInbox->getBody(), true);
        self::assertCount(0, $operatorData['notifications']);
    }

    public function test_submitting_a_report_emails_operators_who_opted_in(): void
    {
        $this->insertUser('operator-no-email@example.com', [
            'is_operator'     => 1,
            'totp_enabled_at' => (new \DateTimeImmutable())->format('Y-m-d H:i:s'),
        ]);
        $this->insertUser('operator-opted-in@example.com', [
            'is_operator'              => 1,
            'totp_enabled_at'          => (new \DateTimeImmutable())->format('Y-m-d H:i:s'),
            'notification_email'       => 'operator-opted-in@example.com',
            'notify_abuse_report_email' => 1,
        ]);
        ...
        self::assertSame('operator-opted-in@example.com', $mailer->sent[0]['to']);
    }
}
