...
final class InviteActionTest extends IntegrationTestCase
{
    ...
    private function postInvite(string $email, ?int $actingUserId, ?string $role = null): ServerRequestInterface
    {
        $csrf   = $this->csrf();
        $token  = $csrf->generate();
    ...
    // -------------------------------------------------------------------------
    // Role selection: defaults to 'member', accepts moderator/admin, rejects
    // anything else (least-privilege default).
    // -------------------------------------------------------------------------

    public function test_invite_without_role_defaults_to_member(): void
    {
        $ownerId = $this->insertUser('owner-invite-default-role@example.com');
        $this->insertAccountMember($this->defaultAccountId(), $ownerId, 'owner');

        $response = $this->createApp()->handle($this->postInvite('default-role@example.com', $ownerId));

        self::assertSame(200, $response->getStatusCode());
        $role = $this->conn->fetchOne(
            'SELECT role FROM invites WHERE account_id = :a ORDER BY id DESC LIMIT 1',
            ['a' => $this->defaultAccountId()],
        );
        self::assertSame('member', $role);
    }

    public function test_invite_with_explicit_admin_role_persists_that_role(): void
    {
        $ownerId = $this->insertUser('owner-invite-admin-role@example.com');
        $this->insertAccountMember($this->defaultAccountId(), $ownerId, 'owner');

        $response = $this->createApp()->handle($this->postInvite('admin-role@example.com', $ownerId, 'admin'));
    ...
    public function test_invite_with_explicit_moderator_role_persists_that_role(): void
    {
        $ownerId = $this->insertUser('owner-invite-moderator-role@example.com');
        $this->insertAccountMember($this->defaultAccountId(), $ownerId, 'owner');

        $response = $this->createApp()->handle($this->postInvite('moderator-role@example.com', $ownerId, 'moderator'));
    ...
    public function test_invite_with_owner_role_is_rejected(): void
    {
        $ownerId = $this->insertUser('owner-invite-owner-role@example.com');
        $this->insertAccountMember($this->defaultAccountId(), $ownerId, 'owner');

        $response = $this->createApp()->handle($this->postInvite('owner-role@example.com', $ownerId, 'owner'));

        self::assertSame(422, $response->getStatusCode());
        $data = json_decode((string) $response->getBody(), true);
        self::assertSame('validation_error', $data['error']['key'] ?? null);
        self::assertArrayHasKey('role', $data['error']['fields'] ?? []);
        self::assertSame(0, (int) $this->conn->fetchOne('SELECT COUNT(*) FROM invites'));
    }

    public function test_invite_with_garbage_role_is_rejected(): void
    {
        $ownerId = $this->insertUser('owner-invite-garbage-role@example.com');
        $this->insertAccountMember($this->defaultAccountId(), $ownerId, 'owner');

        $response = $this->createApp()->handle($this->postInvite('garbage-role@example.com', $ownerId, 'wizard'));
    ...
    /**
     * Regression: EffectivePlan::resolve() used to compare
     * $user['is_operator'] with `=== true` — a raw DB row is a driver-native
     * value (e.g. an int/string 0-or-1 from SQLite/MySQL, never a real PHP
     * bool), so that strict comparison silently NEVER matched and an
     * operator got gated exactly like a regular owner. Fixed to a (bool)
     * cast (core/src/Domain/EffectivePlan.php).
     */
    public function test_operator_bypasses_the_team_limit_even_at_owner_capacity(): void
    {
        $this->setAccountPlan($this->defaultAccountId(), 'starter');
        $ownerId = $this->insertUser('owner-operator-invite@example.com', ['is_operator' => 1]);
        $this->insertAccountMember($this->defaultAccountId(), $ownerId, 'owner'); // already at member_limit=1

        $response = $this->createApp()->handle($this->postInvite('room-for-operator@example.com', $ownerId));
