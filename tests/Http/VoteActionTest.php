...
/**
 * Integration tests for POST /{board}/ideas/{id}/vote.
 *
...
final class VoteActionTest extends IntegrationTestCase
{
    ...
    public function test_first_vote_returns_302_prg_to_idea_detail_and_creates_row(): void
    {
        ...
        // API always returns JSON 200 (no more PRG redirect)
    ...
    public function test_second_same_vote_retracts_no_duplicate_row(): void
    {
        ...
        self::assertSame(0, $this->rowCount($ideaId), 'Casting the same arrow again retracts it (row deleted).');
    ...
    public function test_up_then_down_switches_in_place(): void
    {
        ...
        self::assertSame(1, $this->rowCount($ideaId), 'Switching flips in place, no second row.');
        self::assertSame(-1, $this->scoreCache($ideaId));
    }
    ...
    // --- JSON content-negotiation (Accept:application/json) -----------
    ...
    /** JSON body contains all four fields with correct types. */
    ...
    /** X-Requested-With: XMLHttpRequest also produces a JSON response. */
    ...
    /** JSON path: anon → still 401 (pipeline unchanged). */
    ...
    /** JSON path: missing CSRF → 403 (CSRF middleware unchanged). */
    ...
    /** No JSON Accept header → still JSON 200 (no more content negotiation). */
    public function test_no_json_accept_still_returns_302(): void
    {
        ...
        // VoteAction always returns JSON — no more PRG redirect
        self::assertSame(200, $response->getStatusCode());
        self::assertStringContainsString('application/json', $response->getHeaderLine('Content-Type'));
        $data = json_decode((string) $response->getBody(), true);
        self::assertSame(1, $data['score'] ?? null);
    ...
    // --- Detail-page vote controls (anon → login-links; auth → forms) --

    /** Anon sees is_authenticated=false in the JSON response (SPA renders login links). */
    public function test_idea_detail_renders_login_links_for_anon(): void
    {
        ...
        // No vote state served for anon
        self::assertSame('none', $data['my_vote'] ?? 'none');
    }

    /** Logged-in user sees is_authenticated=true (SPA renders vote forms). */
    public function test_idea_detail_renders_vote_forms_for_authenticated_user(): void
    {
        ...
        self::assertSame(200, $response->getStatusCode());
        $data = json_decode((string) $response->getBody(), true);
        // Auth → is_authenticated=true; SPA renders vote buttons
