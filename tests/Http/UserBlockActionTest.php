...
final class UserBlockActionTest extends IntegrationTestCase
{
    ...
    // -------------------------------------------------------------------------
    // AC1: Moderator blocks a user for a single board only
    // -------------------------------------------------------------------------

    public function test_moderator_blocks_user_for_single_board_returns_200_and_persists_board_scoped(): void
    {
        ...
        $response = $this->createApp()->handle(
            $this->postBlock('block-board-scope', $targetId, true, $modId, 'board'),
        );
