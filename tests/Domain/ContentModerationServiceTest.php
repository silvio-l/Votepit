...
/**
 * Unit tests for ContentModerationService::check (local word filter).
 *
 * Tests exclusively through the public API check() + withAdditionalWords().
 * Table-driven with DataProvider, analogous to TitleNormalizerTest.
 *
 * Tested properties:
 *   - AC1/AC2 — pure domain service, base lists from resources/moderation/ injectable
 *   - AC3 — hits on DE + EN single words and multi-word phrases
 *   - AC4 — whole-word matching: harmless words containing a listed word as a substring do NOT trigger
 *   - AC5 — leetspeak detection
 *   - AC6 — clean text → clean:true / hit → clean:false + hits machine-readable
 *   - AC7 — immutable withAdditionalWords()
 *   - AC8 — table-driven tests; composer qa green
 */
final class ContentModerationServiceTest extends TestCase
{
    ...
    // -------------------------------------------------------------------------
    // AC1 + AC2 — Instantiation + base-list path injectable
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // AC3 — Hits: DE + EN single words and multi-word phrases
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // AC4 — Whole-word matching: Scunthorpe regression
    // -------------------------------------------------------------------------
    ...
    public static function scunthorpeSafeCases(): array
    {
        return [
            ...
            // "anus-exam" without the word alone is fine — test a clear non-match
    ...
    // -------------------------------------------------------------------------
    // AC6 — Clean text → clean:true, hits:[]
    // -------------------------------------------------------------------------
    ...
    public static function cleanInputCases(): array
    {
        return [
            ...
            'German clean sentence' => ['Ich möchte eine bessere Suchfunktion'], // export-ok: comment-language (deliberate German input)
