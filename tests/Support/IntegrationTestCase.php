...
abstract class IntegrationTestCase extends TestCase
{
    ...
    private function applySchema(Connection $conn): void
    {
        ...
        // api_tokens (board-scoped bearer token — see
        // migrations/0010_create_api_tokens.sql for the full rationale).
        $conn->executeStatement(
        ...
        // for the full rationale). SQLite subset: no CHECK-enforced enums,
        // same portability trade-off as abuse_reports/support_requests above.
        $conn->executeStatement(
    ...
    /**
     * Seeds a support_requests row; returns its ID.
     *
     * @param array<string, mixed> $overrides
     */
    protected function insertSupportRequest(int $accountId, int $userId, array $overrides = []): int
    {
        ...
        $this->conn->insert('support_requests', array_merge([
            ...
            'message'    => 'A test message with enough characters.',
            'status'     => 'open',
            'created_at' => $now,
            'updated_at' => $now,
        ], $overrides));
