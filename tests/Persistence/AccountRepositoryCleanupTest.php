...
final class AccountRepositoryCleanupTest extends IntegrationTestCase
{
     ...
     * blocked_users row, an invite, an api_token, an abuse_report, a
     * support_requests ticket (with its opening support_messages row) and
     * an account-scoped notification (with a notification_reads row) — one
     * row per account-scoped core table plus everything cascading
     * transitively via boards.id/notifications.id/support_requests.id.
     *
     * @return array{account_id: int, owner_id: int, member_id: int, board_id: int, idea_id: int, support_request_id: int, notification_id: int}
     */
    private function seedFullAccount(): array
    {
        ...
        $supportRequestId = $this->insertSupportRequest($accountId, $memberId);
        $notificationId   = $this->insertNotification($accountId);
        $this->conn->insert('notification_reads', [
    ...
    public function test_purge_expired_deletes_the_account_and_every_account_scoped_row(): void
    {
        ...
        foreach (['account_members', 'blocked_users', 'invites', 'api_tokens', 'support_requests', 'notifications'] as $table) {
            self::assertSame(
                0,
        ...
        // support_messages/notification_reads cascade transitively via
        // support_requests.id/notifications.id.
        self::assertSame(
            0,
            (int) $this->conn->fetchOne('SELECT COUNT(*) FROM support_messages WHERE request_id = :id', ['id' => $fixture['support_request_id']]),
        );
        self::assertSame(
            0,
            (int) $this->conn->fetchOne('SELECT COUNT(*) FROM notification_reads WHERE notification_id = :id', ['id' => $fixture['notification_id']]),
        );
    ...
    /**
     * notification-preferences feature (Issue 04): `notification_email` and
     * the four flags live on `users`, not on any account-scoped table (a
     * user's platform-wide notification preference must not disappear just
     * because one account they belonged to got deleted — see class doc
     * above on `users` surviving purgeExpired() untouched). Extends the
     * existing "users survive the purge" assertion above with the specific
     * notification-preferences columns.
     */
    public function test_purge_expired_leaves_a_members_notification_preferences_untouched(): void
    {
        $accountId = $this->insertAccount(['slug' => 'to-be-purged-notif']);
        $ownerId   = $this->insertUser('owner-purge-notif@example.com', [
            'notification_email'        => 'owner-purge-notif-inbox@example.com',
            'notify_idea_comment_email' => 1,
        ]);
        $this->insertAccountMember($accountId, $ownerId, 'owner');

        $this->repo()->purgeExpired($accountId);

        $row = $this->conn->fetchAssociative(
            'SELECT notification_email, notify_idea_comment_email FROM users WHERE id = :id',
            ['id' => $ownerId],
        );
        self::assertIsArray($row);
        self::assertSame('owner-purge-notif-inbox@example.com', $row['notification_email']);
