...
/* Rectangular tinted tags, no outline — the landing's tag grammar. */
const chipClass: Record<Status, string> = {
  open: 'bg-vp-ink-soft text-vp-text-secondary',
  planned: 'bg-vp-info-soft text-vp-info-strong',
  'in-progress': 'bg-vp-warn-soft text-vp-warn-strong',
  done: 'bg-vp-vote-up-soft text-vp-vote-up-strong',
  declined: 'bg-vp-vote-down-soft text-vp-vote-down-strong',
}
  ...
  return (
    <span
      className={cx(
        ...
        variant === 'chip'
          ? cx('h-5 px-1.5 rounded-vp-sm leading-none font-semibold', chipClass[status])
    ...
    >
      <span
        className={cx(
          'size-1.5 rounded-vp-full shrink-0',
