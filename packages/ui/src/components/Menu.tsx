 ...
 * links and actions. The panel is an overlay — paper white, the large lift
 * shadow, scaled in from 96%. Arrow keys move between items, Home/End jump,
    ...
    <div ref={rootRef} className={cx('relative', className)} onBlur={onBlur}>
      <button
        ...
        className={buttonClassName(
          ...
          'sm',
          cx('gap-1.5', !hideChevron && 'pr-2', open && 'bg-vp-surface-frost'),
        )}
      >
        {label}
      ...
      {open && (
        <div
          ...
          className={cx(
            'absolute top-full z-50 mt-1.5 min-w-[14rem] max-w-[calc(100vw-2rem)] p-1',
            ...
            if (item.kind === 'separator') {
              return (
                <div
                  ...
                  key={`sep-${i}`}
                  role="separator"
                  className="my-1 h-px bg-vp-border-subtle"
                />
              )
            }
            if (item.kind === 'header') {
