...
/**
 * The one modal confirmation surface, reserved for the step ahead of a
 * destructive action (member removal, token/board/account deletion) — a
 * dialog appearing always means "this is about to change or remove
 * something." A small overlay sheet that scales in from 96%, with a tone
 * glyph in the head. Manual overlay (same pattern as `Menu`, not the native
...
export function ConfirmDialog({
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-vp-ink/50 backdrop-blur-[2px] p-4 animate-vp-fade-in"
      onMouseDown={(e) => {
    ...
    >
      ...
      >
          ...
          <span
            ...
            className={cx(
              ...
              danger
                ...
                : 'bg-vp-surface-frost text-vp-text-secondary ring-1 ring-vp-border-subtle',
            )}
          >
            {danger ? <DangerGlyph /> : <QuestionGlyph />}
          </span>
          <div className="flex flex-col gap-1.5 min-w-0 pt-1">
            <h2 id={titleId} className="text-vp-lg font-semibold text-vp-ink tracking-[-0.01em]">
              {title}
            </h2>
            {description && (
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 border-t border-vp-border-subtle bg-vp-surface-frost px-5 py-3">
