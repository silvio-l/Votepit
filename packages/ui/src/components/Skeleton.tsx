...
export function SkeletonRows({ rows = 4, variant = 'list', className }: SkeletonRowsProps) {
  ...
  return (
    <div aria-hidden="true" className={cx('flex flex-col divide-y divide-vp-border-subtle', className)}>
      {Array.from({ length: rows }, (_, i) => (
        <div key={i} className={cx('flex gap-4', ballot ? 'items-start py-3' : 'items-center py-3.5')}>
          ...
          <Skeleton className={cx('hidden sm:block', ballot ? 'h-2 w-36 self-center rounded-full' : 'h-3 w-20')} />
        </div>
      ))}
    </div>
  )
