...
function Glyph({ kind }: { kind: NonNullable<ErrorStateProps['kind']> }) {
  ...
  if (kind === 'missing') {
    return (
      <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <rect x="5" y="3" width="14" height="18" rx="2" strokeDasharray="3 2.5" />
        <path d="M9 12h6" />
      </svg>
    )
  }
  ...
  return (
    ...
    >
      <span
        aria-hidden="true"
        className={cx(
          'flex items-center justify-center size-14 rounded-vp-xl ring-1',
          calm
            ? 'bg-vp-surface-frost ring-vp-border-subtle text-vp-text-muted'
            : 'bg-vp-vote-down-soft ring-vp-vote-down/30 text-vp-vote-down-strong',
        )}
      >
        <Glyph kind={kind} />
      </span>
      <h2 className="text-vp-lg font-semibold text-vp-ink tracking-[-0.01em] mt-1">{title}</h2>
      {description && (
