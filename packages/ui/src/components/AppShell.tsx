...
/** The sidebar's navigation list: grouped, icon + label, current item raised. */
...
interface AppShellProps {
  ...
  /** Content width: "default" (max 1120px) or "narrow" (forms, 44rem). */
...
export function AppShell({
  ...
  const sidebarInner = (
    <div className="flex h-full flex-col">
      ...
      {sidebarFooter && (
        <div className="border-t border-vp-border-subtle px-3 py-3">{sidebarFooter}</div>
      )}
    </div>
  )
    ...
    <div className="min-h-screen flex flex-col vp-desk text-vp-ink">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside
          ...
          className="hidden lg:block sticky top-vp-topbar h-[calc(100vh-var(--spacing-vp-topbar))] w-vp-sidebar shrink-0 border-r border-vp-border-subtle bg-vp-surface-frost/60"
        >
          {sidebarInner}
        </aside>
        ...
        {open && (
          <div className="lg:hidden fixed inset-0 z-50">
            ...
            <aside
              id={drawerId}
              aria-label={sidebarAriaLabel}
              className="absolute inset-y-0 left-0 w-[min(18rem,85vw)] bg-vp-surface shadow-vp-lg animate-vp-slide-in-left flex flex-col"
            >
              <div className="flex items-center justify-end px-3 h-vp-topbar border-b border-vp-border-subtle">
                <button
                  type="button"
                  onClick={() => {
                    setOpen(false)
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          <main
            id="main"
            className={cx(
              'flex-1 w-full mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-8',
          ...
          >
            <div className="lg:hidden mb-4">
              <button
                ...
                className="inline-flex items-center gap-2 h-9 px-3 rounded-vp-md border border-vp-border-strong bg-vp-surface text-vp-sm font-medium text-vp-ink shadow-[0_1px_2px_rgba(21,22,26,0.04)] hover:bg-vp-surface-frost cursor-pointer vp-press"
              >
                <MenuGlyph />
                {openSidebarLabel}
              </button>
          ...
          {legalLinks.length > 0 && (
            <div className="px-4 sm:px-6 lg:px-8 border-t border-vp-border-subtle">
