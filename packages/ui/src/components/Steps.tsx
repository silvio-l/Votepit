...
/**
 * Wizard progress: numbered square markers joined by a rule; done steps
 * stamp a check, the current one is ink, the rest wait in outline.
 */
          ...
          <li key={item.label} className={cx('flex items-center gap-2 min-w-0', i < items.length - 1 && 'flex-1')}>
            <span className="flex items-center gap-2 min-w-0">
              <span
                aria-hidden="true"
                className={cx(
                  'flex size-6 shrink-0 items-center justify-center rounded-vp-sm text-vp-2xs font-semibold font-mono-num transition-colors duration-200',
                  done && 'bg-vp-vote-up text-white',
                  active && 'bg-vp-ink text-vp-on-ink',
            ...
            {i < items.length - 1 && (
              <span
                aria-hidden="true"
                className={cx('h-px flex-1 min-w-4', done ? 'bg-vp-vote-up' : 'bg-vp-rule')}
              />
            )}
          </li>
        )
