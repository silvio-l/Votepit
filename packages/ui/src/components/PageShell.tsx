    ...
    >
      {legalLinks.map((link) => (
        <a
          ...
          className="no-underline hover:text-vp-ink hover:underline decoration-2 underline-offset-[0.2em] decoration-vp-underline transition-colors duration-150"
        >
          {link.label}
        </a>
      ))}
...
export function PageShell({ children, header, legalLinks = [], width = 'default' }: PageShellProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col vp-desk text-vp-ink">
      ...
      {legalLinks.length > 0 && (
        <div className={cx(containerClass, 'border-t border-vp-rule')}>
