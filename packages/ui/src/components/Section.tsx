...
/**
 * A white plane on the paper: one hairline, head / body / foot, square
 * corners, no shadow. Every admin form, list and result block lives on one
 * of these — the only container the app has, so nothing is ever nested
 * inside another card. Titles are set in Archivo like the landing's
 * section heads, so the two surfaces share one heading voice.
 */
  ...
  return (
    <section
      ...
      className={cx(
        ...
        emphasis === 'danger' && 'border-t-[3px] border-t-vp-vote-down',
        className,
      )}
    >
      {hasHead && (
        <div
          className={cx(
            'flex flex-wrap items-start justify-between gap-x-6 gap-y-2 px-4 sm:px-5 py-3.5 border-b border-vp-rule',
        ...
        >
          <div className="min-w-0 flex-1">
            {title && (
              <Heading
                id={headingId}
                className="flex items-center gap-2 font-archivo font-bold text-vp-lg text-vp-ink leading-6 tracking-[-0.02em]"
              >
                {icon && (
                  <span
                    aria-hidden="true"
      ...
      {footer && (
        <div className="flex flex-wrap items-center gap-3 px-4 sm:px-5 py-3 border-t border-vp-rule bg-vp-surface-frost">
          {footer}
        </div>
      )}
    </section>
