 ...
 * by the operator.
 *
 * Dead-man's-switch (deep-review-2026-09 finding e): records a heartbeat via
 * CronHeartbeatRepository on every run — see
 * bin/cleanup-expired-accounts.php's class doc and bin/check-cron-heartbeats.php.
 *
 * Usage:
...
try {
    ...
    (new \Votepit\Persistence\CronHeartbeatRepository($conn))->recordFailure('cleanup-rate-limits', $e->getMessage());
    fwrite(STDERR, 'Votepit: rate_limits cleanup failed (' . $e->getMessage() . ").\n");
    exit(1);
}

echo sprintf("Done — %d expired rate_limits bucket(s) deleted.\n", $deleted);

(new \Votepit\Persistence\CronHeartbeatRepository($conn))->recordSuccess('cleanup-rate-limits', sprintf('%d deleted', $deleted));
