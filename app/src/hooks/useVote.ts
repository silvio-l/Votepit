...
export interface UseVoteOptions {
  ...
  returnTo?: string
  /**
   * Called after the optimistic state has been rolled back because the server
   * refused the vote. Without it the mark would simply snap back with no
   * explanation — the page uses this to raise a toast.
   */
  onError?: () => void
}
...
export function toUserVote(v: 'up' | 'down' | 'none' | undefined): UserVote {
  if (v === 'up') return 'up'
  if (v === 'down') return 'down'
  return null
}

export function fromServerResponse(r: VoteResponse): VoteState {
  return {
    score: r.score,
 ...
 */
export function computeOptimistic(current: VoteState, direction: 'up' | 'down'): VoteState {
 ...
 *   4. ROLLS BACK to the pre-click state on any error and reports it via
 *      `onError`, so the page can tell the voter the mark did not stick.
 *
  ...
  const handleVote = useCallback(
    async (direction: 'up' | 'down') => {
      ...
      try {
        ...
        // Activation goal (measurement plan: "first_vote_cast", sites 10/11) —
        // fired on every fresh cast (before.myVote === null), not on a
        // retraction/flip of an existing vote. "First" is Matomo's own
        // per-visitor unique-conversion count, not something tracked here.
    ...
    [boardSlug, ideaId, isAuthenticated, location.pathname, navigate, returnTo, onError],
  )
