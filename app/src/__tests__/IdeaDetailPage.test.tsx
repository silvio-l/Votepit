...
/** SQL-style "Y-m-d H:i:s" timestamp, matching the API's created_at format. */
...
function makeDetailResponse(
  ...
  return {
    ...
    idea,
    comments: comments.map((c) => ({ edited_at: null, ...c })),
...
function renderDetailPageAt(entry: string) {
  return render(
...
describe('IdeaDetailPage — admin block control', () => {
  ...
  it('admin blocks the author: API POST called with user_id + blocked:true, button label updates', async () => {
    ...
    await user.click(within(dialog).getByRole('button', { name: 'Autor sperren (Account)' }))
  ...
  it('admin picks board scope: API POST called with scope:board, button label reflects the board scope', async () => {
    ...
    const dialog = await screen.findByRole('alertdialog')
    await user.click(within(dialog).getByRole('button', { name: 'Autor sperren (Board)' }))
...
describe('IdeaDetailPage — admin comment moderation', () => {
  ...
  it('admin sees an "Autor sperren" button next to a comment', async () => {
    const idea = makeIdea()
    makeFetchMock([
      { body: ADMIN_BOOTSTRAP },
      { body: makeDetailResponse(idea, true, seededComments) },
    ...
    await waitFor(() => expect(screen.getByText('Ein Kommentar')).toBeInTheDocument())
    expect(screen.getByRole('button', { name: 'Autor sperren' })).toBeInTheDocument()
  })

  it('non-admin does NOT see an "Autor sperren" button next to a comment', async () => {
    const idea = makeIdea()
    makeFetchMock([
      { body: USER_BOOTSTRAP },
      { body: makeDetailResponse(idea, true, seededComments) },
    ...
    await waitFor(() => expect(screen.getByText('Ein Kommentar')).toBeInTheDocument())
    expect(screen.queryByRole('button', { name: 'Autor sperren' })).not.toBeInTheDocument()
  })

  it("admin blocks a comment author: API POST called with the comment author's id, button flips to blocked", async () => {
    const user = userEvent.setup()
    const idea = makeIdea()
    ...
    await user.click(screen.getByRole('button', { name: 'Autor sperren' }))

    const dialog = await screen.findByRole('alertdialog')
    await user.click(within(dialog).getByRole('button', { name: 'Autor sperren' }))

    await waitFor(() =>
      expect(screen.getByRole('button', { name: 'Kommentar-Autor gesperrt' })).toBeInTheDocument(),
    )
    const blockCall = fetchCalls.find((c) => c.url.includes('/block'))
    expect(blockCall?.url).toContain('/demo/block')
    expect(JSON.parse(blockCall?.body ?? '{}')).toMatchObject({
      user_id: 99,
      blocked: true,
      scope: 'account',
    })
...
describe('IdeaDetailPage — comment editing (AC6)', () => {
  it('author sees a "Bearbeiten" button next to their own recent comment', async () => {
    const idea = makeIdea()
    const ownComment = [
      {
        id: 1,
    ...
    makeFetchMock([{ body: USER_BOOTSTRAP }, { body: makeDetailResponse(idea, true, ownComment) }])

    renderDetailPage()

    await waitFor(() => expect(screen.getByText('Mein Kommentar')).toBeInTheDocument())
    expect(screen.getByRole('button', { name: 'Bearbeiten' })).toBeInTheDocument()
  })

  it('does NOT show a "Bearbeiten" button next to someone else\'s comment', async () => {
    const idea = makeIdea()
    const othersComment = [
      {
        id: 1,
    ...
    makeFetchMock([
      { body: USER_BOOTSTRAP },
      { body: makeDetailResponse(idea, true, othersComment) },
    ])

    renderDetailPage()

    await waitFor(() => expect(screen.getByText('Fremder Kommentar')).toBeInTheDocument())
    expect(screen.queryByRole('button', { name: 'Bearbeiten' })).not.toBeInTheDocument()
  })

  it('does NOT show a "Bearbeiten" button once the edit window has elapsed', async () => {
    const idea = makeIdea()
    const oldOwnComment = [
      {
        id: 1,
        idea_id: 42,
        author_id: 2,
        body: 'Alter Kommentar',
        created_at: sqlTimestamp(new Date(Date.now() - 90_000)),
      },
    ]
    makeFetchMock([
      { body: USER_BOOTSTRAP },
      { body: makeDetailResponse(idea, true, oldOwnComment) },
    ])

    renderDetailPage()

    await waitFor(() => expect(screen.getByText('Alter Kommentar')).toBeInTheDocument())
    expect(screen.queryByRole('button', { name: 'Bearbeiten' })).not.toBeInTheDocument()
  })

  it('editing and saving updates the comment body and shows the "bearbeitet" indicator', async () => {
    const user = userEvent.setup()
    const idea = makeIdea()
    const ownComment = [
      {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      const url = typeof input === 'string' ? input : (input as Request).url
      if (url.includes('/api/bootstrap')) {
        return new Response(JSON.stringify(USER_BOOTSTRAP), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        })
      }
      if (url.includes('/comments/1/edit')) {
        return new Response(JSON.stringify({ ok: true }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        })
      }
      return new Response(JSON.stringify(makeDetailResponse(idea, true, ownComment)), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      })
    })
    ...
    await waitFor(() => expect(screen.getByText('Originaltext')).toBeInTheDocument())
    await user.click(screen.getByRole('button', { name: 'Bearbeiten' }))

    const editField = screen.getByLabelText('Kommentar bearbeiten')
    await user.clear(editField)
    await user.type(editField, 'Korrigierter Text')
    await user.click(screen.getByRole('button', { name: 'Speichern' }))

    await waitFor(() => expect(screen.getByText('Korrigierter Text')).toBeInTheDocument())
    expect(screen.queryByText('Originaltext')).not.toBeInTheDocument()
    expect(screen.getByText('bearbeitet')).toBeInTheDocument()
  })

  it('a server error on save shows an inline message and keeps the edit form open', async () => {
    const user = userEvent.setup()
    const idea = makeIdea()
    const ownComment = [
      {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (url.includes('/comments/1/edit')) {
        return new Response(
          JSON.stringify({
            error: { key: 'edit_window_expired', message: 'Zu spät zum Bearbeiten.' },
          }),
          { status: 422, headers: { 'Content-Type': 'application/json' } },
        )
      }
    ...
    await waitFor(() =>
      expect(screen.getByRole('alert')).toHaveTextContent('Zu spät zum Bearbeiten.'),
    )
    // Form stays open with the unsaved edit still visible.
    expect(screen.getByLabelText('Kommentar bearbeiten')).toBeInTheDocument()
  })

  it('cancelling an edit discards the draft and restores the original text', async () => {
    const user = userEvent.setup()
    const idea = makeIdea()
    const ownComment = [
      {
    ...
    const editField = screen.getByLabelText('Kommentar bearbeiten')
    await user.clear(editField)
    await user.type(editField, 'Verworfener Entwurf')
    await user.click(screen.getByRole('button', { name: 'Abbrechen' }))

    expect(screen.getByText('Originaltext')).toBeInTheDocument()
    expect(screen.queryByText('Verworfener Entwurf')).not.toBeInTheDocument()
  })
})

describe('IdeaDetailPage — voter-preview toggle', () => {
  const seededComments = [
    { id: 1, idea_id: 42, author_id: 99, body: 'Ein Kommentar', created_at: '2025-06-01 10:00:00' },
  ]

  it('admin sees the preview switch and, toggling it, every inline moderation control disappears', async () => {
    const user = userEvent.setup()
    const idea = makeIdea()
    makeFetchMock([
      { body: ADMIN_BOOTSTRAP },
    ...
    // Moderation controls are visible before the toggle.
    expect(screen.getByRole('combobox', { name: 'Status ändern' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Anpinnen' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Entfernen' })).toBeInTheDocument()
    ...
    expect(screen.queryByRole('button', { name: 'Anpinnen' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Entfernen' })).not.toBeInTheDocument()
    // The header's admin affordances are hidden too.
    expect(screen.queryByRole('link', { name: 'Einstellungen' })).not.toBeInTheDocument()

    // Toggling back restores every control.
  ...
  it('non-admin never sees the voter-preview toggle', async () => {
    const idea = makeIdea()
    makeFetchMock([{ body: USER_BOOTSTRAP }, { body: makeDetailResponse(idea) }])
    ...
    expect(screen.queryByRole('menuitem', { name: 'Als Voter ansehen' })).not.toBeInTheDocument()
  })

  it('landing with ?view=voter starts in preview mode — moderation controls are hidden from the start', async () => {
    const idea = makeIdea()
    makeFetchMock([
      { body: ADMIN_BOOTSTRAP },
      { body: makeDetailResponse(idea, true, seededComments) },
    ...
    renderDetailPageAt('/demo/idea/42?view=voter')
  ...
  it('preview mode carried over from the module store (simulating in-app navigation from another board page)', async () => {
