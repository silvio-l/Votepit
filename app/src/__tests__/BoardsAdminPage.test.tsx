...
const BOARDS_RESPONSE = {
  ...
  account: {
    onboarding_completed_at: '2020-01-01T00:00:00+00:00',
...
describe('BoardsAdminPage — board list', () => {
  ...
  it('shows API tokens as a top-level sidebar item, not just a board-scoped link', async () => {
    makeFetchMock([{ body: ADMIN_BOOTSTRAP }, { body: BOARDS_RESPONSE }])
    ...
    expect(tokensLink.getAttribute('href')).toBe('/admin/tokens')
  })
  ...
  it('copies the full board link to the clipboard', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /Board-Link kopieren: Alpha Board/i }))

    expect(writeText).toHaveBeenCalledWith(`${window.location.origin}/alpha`)
    await waitFor(() =>
...
describe('BoardsAdminPage — create-form: visibility selector', () => {
  it('pre-selects the safest plan-allowed visibility and submits it to the API', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      capturedBody = init?.body !== undefined ? JSON.parse(init.body as string) : null
      return new Response(
        JSON.stringify({ ok: true, slug: 'new-board', name: 'New Board', visibility: 'private' }),
    ...
    // BOARDS_RESPONSE.account.default_visibility is 'private'.
    const visibilitySelect = screen.getByLabelText(/Sichtbarkeit/i) as HTMLSelectElement
    ...
    expect((capturedBody as { visibility: string }).visibility).toBe('private')
  })

  it('disables visibility options the current plan does not allow', async () => {
    makeFetchMock([
      { body: ADMIN_BOOTSTRAP },
      {
        body: {
    ...
    const unlistedOption = screen.getByRole('option', {
      name: /Nicht gelistet/i,
    }) as HTMLOptionElement
    const privateOption = screen.getByRole('option', { name: /^Privat/i }) as HTMLOptionElement
    expect(unlistedOption.disabled).toBe(true)
    expect(privateOption.disabled).toBe(true)
  })

  it('lets the user pick a different allowed visibility before submitting', async () => {
    let capturedBody: unknown = null
    let callIndex = 0
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      if (typeof input === 'string' && input.startsWith('/notifications')) {
      ...
      capturedBody = init?.body !== undefined ? JSON.parse(init.body as string) : null
      return new Response(
        JSON.stringify({ ok: true, slug: 'new-board', name: 'New Board', visibility: 'public' }),
        { status: 201, headers: { 'Content-Type': 'application/json' } },
      )
    })
    ...
    const user = userEvent.setup()
    await user.selectOptions(screen.getByLabelText(/Sichtbarkeit/i), 'public')
    await user.click(await screen.findByRole('button', { name: /Öffentlich machen/i }))
    await user.type(screen.getByLabelText(/^Name/i), 'New Board')
    await user.click(screen.getByRole('button', { name: /Board anlegen/i }))

    await waitFor(() => expect(capturedBody).not.toBeNull())
    expect((capturedBody as { visibility: string }).visibility).toBe('public')
  })

  it('keeps the previous visibility when the confirmation is cancelled', async () => {
    makeFetchMock([{ body: ADMIN_BOOTSTRAP }, { body: BOARDS_RESPONSE }])
