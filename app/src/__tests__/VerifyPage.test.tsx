...
describe('VerifyPage', () => {
  ...
  it('shows the 2FA step instead of navigating when the server returns requires_2fa', async () => {
    const responses = [
      new Response(JSON.stringify({ requires_2fa: true, pending_token: 'pending-xyz' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
      new Response(JSON.stringify({ ok: true, redirect: '/demo' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
    ]
