...
const NOTIFICATION_PREFS_NOTIFICATIONS_RESPONSE = {
  notifications: [
    {
      id: 3,
      scope: 'user',
      type: 'idea_comment',
      title: 'Neuer Kommentar auf deiner Idee',
      body: 'Jemand hat auf "Meine Idee" kommentiert.',
      link_path: '/ideas/1',
    ...
    },
    {
      ...
      scope: 'user',
      type: 'thread_reply',
      title: 'Neue Antwort in einem Thread',
      body: 'Jemand hat auf einen Kommentar geantwortet.',
      link_path: '/ideas/1',
...
describe('InboxPage — notification-preferences types (idea_comment / thread_reply)', () => {
  it('renders idea_comment and thread_reply notifications with distinct icons from support_reply/announcement', async () => {
    makeFetchMock([{ body: BOOTSTRAP_OK }, { body: NOTIFICATION_PREFS_NOTIFICATIONS_RESPONSE }])

    renderInboxPage()

    await waitFor(() =>
      expect(screen.getByText('Neuer Kommentar auf deiner Idee')).toBeInTheDocument(),
    )
    expect(screen.getByText('Neue Antwort in einem Thread')).toBeInTheDocument()

    const ideaCommentRow = screen.getByText('Neuer Kommentar auf deiner Idee').closest('li')
    const threadReplyRow = screen.getByText('Neue Antwort in einem Thread').closest('li')
    if (ideaCommentRow === null || threadReplyRow === null) {
      throw new Error('notification rows not found')
    }
    expect(ideaCommentRow.querySelector('svg.lucide-message-circle')).not.toBeNull()
    expect(threadReplyRow.querySelector('svg.lucide-reply')).not.toBeNull()
  })
})
