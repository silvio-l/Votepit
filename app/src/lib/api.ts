...
/** Board-wide "this week" aggregates for the FeaturedIdeaCard. */
 ...
 */
export function accountRoleFor(
  user: User | null,
...
export async function completeOnboarding(): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>('POST', accountPath('/admin/onboarding/complete'), {})
}
 ...
 * name, invalid/reserved/colliding slug) → ApiError(422) with payload.fields.
 */
export async function createAdminBoard(
  payload: CreateAdminBoardPayload,
): Promise<CreateAdminBoardResponse> {
  return request<CreateAdminBoardResponse>('POST', accountPath('/admin/boards'), payload)
}
...
export type AccountRole = 'owner' | 'moderator'
...
/**
 * POST /admin/invites — invite a new member by email. Always becomes
 * 'moderator' on acceptance (server-enforced — owners are never created via
 * invite).
 ...
 * self-invite. Requires CSRF — call bootstrap() first.
 */
export async function inviteMember(email: string): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>('POST', accountPath('/admin/invites'), { email })
}
...
/**
 * POST /admin/members/{userId}/role — change a member's role (owner|moderator).
 * AuthZ: accountOwner. Throws ApiError(422, key='last_owner') when demoting
 * the account's last owner.
 */
...
/**
 * GET /admin/boards/{slug}/tokens — list this board's API tokens (never
 * includes the token hash or plaintext).
 * AuthZ: accountAdmin (owner AND moderator) — throws ApiError(401) for anon,
 * ApiError(404) for an unknown/foreign-account board slug.
 */
export async function listApiTokens(boardSlug: string): Promise<ApiTokenListResponse> {
  return request<ApiTokenListResponse>('GET', accountPath(`/admin/boards/${boardSlug}/tokens`))
}
...
/**
 * POST /admin/boards/{slug}/tokens — create a new board-scoped API token.
 ...
 * AuthZ: accountAdmin. Throws ApiError(422) with fields.label for an empty/
 * too-long label. Requires CSRF — call bootstrap() first.
 */
export async function createApiToken(
  boardSlug: string,
  label: string,
): Promise<CreateApiTokenResponse> {
  return request<CreateApiTokenResponse>('POST', accountPath(`/admin/boards/${boardSlug}/tokens`), {
    label,
  })
}

/**
 * POST /admin/boards/{slug}/tokens/{id}/revoke — revoke an API token.
 * AuthZ: accountAdmin. Throws ApiError(404) for an unknown/foreign-board token id.
 */
export async function revokeApiToken(boardSlug: string, tokenId: number): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(
    'POST',
    accountPath(`/admin/boards/${boardSlug}/tokens/${tokenId}/revoke`),
    {},
  )
}
...
export async function createSignupAccount(
  ...
  return request<CreateSignupAccountResponse>('POST', '/signup/account', payload)
}
...
/** GET /operator/support — every account's support tickets. */
export async function listOperatorSupportRequests(): Promise<{
  requests: SupportRequestSummary[]
}> {
  return request<{ requests: SupportRequestSummary[] }>('GET', '/operator/support')
}
...
// migrations/0024_add_notifications_remove_support_email.sql. Two kinds share
// one shape: a 'support_reply' notification (account-scoped, system-
// generated) and an 'announcement' notification (broadcast, operator-
// authored — see the Operator panel section below).

export type NotificationType = 'support_reply' | 'announcement'
export type NotificationScope = 'account' | 'broadcast'
