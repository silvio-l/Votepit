...
import { Alert, Button, DescriptionList, Section, TextInput } from '@votepit/ui'
...
function PasswordSection({
  ...
  return (
    ...
    >
      ...
      <PasswordResetLinkAffordance t={t} />
    </Section>
  )
}

function PasswordResetLinkAffordance({ t }: { t: Translator }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSuccess(false)
    try {
      await requestOwnPasswordReset(email.trim())
      setSuccess(true)
      setEmail('')
    } catch (err) {
      const apiErr = err as ApiError
      setError(
        apiErr?.payload?.key === 'email_mismatch'
          ? t('passwordResetLinkMismatchError')
          : (apiErr?.payload?.message ?? t('passwordResetLinkGenericError')),
      )
    } finally {
      setSubmitting(false)
    }
  ...
  if (!open) {
    return (
      ...
      >
        {t('passwordResetLinkCta')}
      </Button>
    )
  }

  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-3 max-w-sm mt-1">
      <TextInput
        label={t('passwordResetLinkEmailLabel')}
        type="email"
        id="password-reset-link-email"
        value={email}
        onChange={setEmail}
        hint={t('passwordResetLinkEmailHint')}
        required
        disabled={submitting}
        autoComplete="email"
      />
      ...
      {error && <Alert tone="error">{error}</Alert>}
      {success && <Alert tone="success">{t('passwordResetLinkSuccess')}</Alert>}
      ...
      <div className="flex gap-2">
        <Button
          ...
          loading={submitting}
        >
          {submitting ? t('passwordResetLinkSending') : t('passwordResetLinkSendCta')}
        </Button>
        <Button
          type="button"
          variant="ghost"
...
function TotpSetupForm({
  ...
  return (
    <form onSubmit={handleConfirm} noValidate className="flex flex-col gap-4 max-w-sm">
      ...
      <DescriptionList
        items={[{ term: t('totpManualEntryHint'), detail: state.secret, mono: true }]}
      />
...
function BackupCodesReveal({
  ...
  return (
    <div className="flex flex-col gap-4 max-w-sm">
      ...
      <ul className="grid grid-cols-2 gap-2 font-mono-num text-vp-sm text-vp-ink bg-vp-surface-frost border border-vp-rule rounded-vp-md p-3">
        {codes.map((c) => (
