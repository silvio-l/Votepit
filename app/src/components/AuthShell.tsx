...
import { Button, cx, LegalFooter, Spinner, VotepitLogo } from '@votepit/ui'
  ...
  return (
    ...
    >
      <g strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
        <rect x="1" y="1" width="26" height="26" rx="6" stroke="var(--color-vp-vote-up)" />
        ...
        <rect x="1" y="39" width="26" height="26" rx="6" stroke="var(--color-vp-rule-on-ink)" />
        ...
        <rect x="1" y="77" width="26" height="26" rx="6" stroke="var(--color-vp-vote-down)" />
...
function LanguageToggle() {
  ...
  return (
    <div
      className="inline-flex items-center gap-0.5 rounded-vp-md border border-vp-border-subtle bg-vp-surface p-0.5"
      role="group"
    ...
    >
      {LANGUAGES.map((lang) => (
        <Button
          ...
          className={cx(
            'uppercase tracking-[0.08em] font-mono-num min-w-9',
            language === lang
              ? 'bg-vp-ink text-vp-on-ink hover:bg-vp-ink hover:text-vp-on-ink'
              : 'text-vp-text-muted',
          )}
        >
          {lang}
    ...
    <div className="min-h-screen vp-desk flex flex-col lg:flex-row">
      ...
      >
        ...
        <div className="relative z-[1] flex flex-col gap-8 max-w-md">
          ...
          <h2 className="font-archivo font-bold text-vp-3xl xl:text-vp-4xl tracking-[-0.025em] leading-[1.05] text-vp-on-ink text-balance">
            {t('auth.tagline')}
          </h2>
        ...
        <div className="relative z-[1] flex flex-wrap items-center gap-x-4 gap-y-1 text-vp-xs text-vp-on-ink-muted">
          ...
          {legalLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="no-underline hover:text-vp-on-ink hover:underline"
            >
              {link.label}
            </a>
          ))}
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center px-4 py-8 sm:py-12">
          <div className="w-full max-w-[26rem]">
            ...
            <div className="vp-card vp-sheet--ruled p-6 sm:p-8 animate-vp-rise">{children}</div>
            {footer && (
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        {title && (
          <Heading className="font-archivo font-bold text-vp-2xl tracking-[-0.02em] text-vp-ink text-balance">
            {title}
          </Heading>
        )}
        {children && (
...
function OutcomeMark({ tone }: { tone: AuthOutcomeProps['tone'] }) {
  if (tone === 'pending') {
    return (
...
export function AuthHeading({ title, intro }: { title: string; intro?: string }) {
  return (
    <div className="mb-6">
      <h1 className="font-archivo font-bold text-vp-2xl tracking-[-0.02em] text-vp-ink text-balance">
        {title}
      </h1>
