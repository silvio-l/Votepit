...
import { HighestRoleBadge } from '../components/RoleBadge'
...
/**
 * Contribution stats (social-features issue 06) — the three account-scoped
 * numbers the server sends alongside a visible profile. Always present when
 * visible: true (0 for no activity), never sent when visible: false.
 */
function ContributionStats({
  ...
  const stats: { label: string; value: number }[] = [
    { label: t('statsIdeasSubmitted'), value: ideasSubmitted },
    { label: t('statsIdeasShipped'), value: ideasShipped },
    { label: t('statsVotesCast'), value: votesCast },
  ]
    ...
    <dl className="flex flex-wrap gap-x-6 gap-y-2">
      {stats.map(({ label, value }) => (
        <div key={label} className="flex items-baseline gap-1.5">
          <dt className="text-vp-sm text-vp-text-secondary">{label}</dt>
          <dd className="font-mono-num text-vp-sm font-semibold text-vp-ink">{value}</dd>
        </div>
      ))}
    </dl>
  )
        ...
        return (
          <li key={field}>
            <a
              ...
              className="inline-flex items-center gap-1.5 h-8 rounded-vp-sm border border-vp-rule bg-vp-surface px-3 text-vp-sm font-medium text-vp-text-secondary no-underline transition-colors duration-150 hover:border-vp-ink hover:text-vp-ink"
            >
      ...
      <Card padding="lg" className="vp-sheet--ruled animate-vp-rise">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-7">
          <Avatar
            ...
            size={96}
            alt=""
            className="shrink-0 self-start ring-1 ring-vp-rule"
          />
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <HighestRoleBadge isOperator={profile.is_operator} role={profile.role} />
            </div>

            {profile.visible ? (
              <>
                <ContributionStats
                  ideasSubmitted={profile.ideas_submitted}
                  ideasShipped={profile.ideas_shipped}
                  votesCast={profile.votes_cast}
                />
