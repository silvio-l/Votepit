...
import type { AdminBoardSummary, ApiError, User } from '../lib/api'
import {
  bootstrap,
  completeOnboarding,
  createAdminBoard,
...
type PageState =
  ...
  | { phase: 'ready'; boards: AdminBoardSummary[]; onboardingCompleted: boolean }
...
export default function BoardsAdminPage() {
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createAdminBoard({ name: newName.trim(), slug: newSlug.trim() })
  ...
  const { boards, onboardingCompleted } = pageState

  if (!onboardingCompleted) {
    return frame(<SetupWizard boards={boards} onDone={handleWizardDone} />, 'narrow')
  }
                    ...
                    <Card interactive padding="none" className="h-full flex flex-col">
                      ...
                      <div className="mt-auto flex items-center gap-1 border-t border-vp-border-subtle bg-vp-surface-frost px-2 py-1.5 rounded-b-[inherit]">
                        ...
                        <Link
                          ...
                          className={buttonClassName('ghost', 'sm', 'ml-auto')}
