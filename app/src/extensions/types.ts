...
/**
 * A sidebar entry in the platform-operator area of the admin shell
 * (AdminShell `area="operator"`), next to core's own operator panel — the
 * analogue of ExtensionAdminNavLink for global routes. Listed before the
 * operator panel, in the order given, so an extension's overview page can be
 * the area's natural landing.
 */
interface ExtensionPlatformNavLink {
  /** Absolute route path, e.g. `/admin/overview` — one of `globalRoutes`. */
  path: string
  label: Record<Language, string>
  /** 16px glyph; core falls back to a generic one. */
  icon?: ReactElement
}
...
export interface AppExtensions {
  ...
  /** Sidebar entries of the platform-operator area, listed before core's operator panel. */
  platformNavLinks: ExtensionPlatformNavLink[]
